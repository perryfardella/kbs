...
// Outer shell: waits for both rule and categories to load before mounting the form
export function EditRecurringForm({ recurringTransactionId, onSuccess }: EditRecurringFormProps) {
  const rule = useQuery(api.recurringTransactions.get, { recurringTransactionId });
  const categories = useQuery(api.categories.list);

  if (rule === undefined || categories === undefined) {
    return (
...
type RuleData = NonNullable<ReturnType<typeof useQuery<typeof api.recurringTransactions.get>>>;
...
// Inner form: receives rule + categories as props, uses them as static defaultValues
function EditRecurringFormInner({ recurringTransactionId, rule, categories, onSuccess }: InnerProps) {
